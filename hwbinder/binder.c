 ...
 */
#define BINDER_DKMS_MAJOR 510
#define BINDER_DKMS_MINOR 0
...
static int binder_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (proc->tsk != current && proc->tsk != current->group_leader)
		return -EINVAL;
...
static int __init init_binder_device(int idx)
{
	...
	devnr = MKDEV(BINDER_DKMS_MAJOR, BINDER_DKMS_MINOR);
	...
	if (binder_devices_param > 1)
		ret = snprintf(numstr, sizeof(numstr), "hwbinder%d", idx);
...
static int __init binder_init(void)
{
	...
	binder_debugfs_dir_entry_root = debugfs_create_dir("hwbinder", NULL);
	...
	ret = register_chrdev_region(MKDEV(BINDER_DKMS_MAJOR, BINDER_DKMS_MINOR),
				     BINDER_DKMS_MAX_MINOR, "hwbinder");
	if (ret)
		goto on_error_remove_debugfs;

	binder_class = class_create(THIS_MODULE, "hwbinder");
...
on_error_unregister_chrdev_region:
	unregister_chrdev_region(MKDEV(BINDER_DKMS_MAJOR, BINDER_DKMS_MINOR),
